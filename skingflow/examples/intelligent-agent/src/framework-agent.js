/**
 * Framework-based Intelligent Agent System
 * 
...
class KnowledgeProcessor extends AsyncNode {
  ...
  async *execAsyncStream(shared) {
    ...
    try {
      const memories = await this.framework.memory.search(query, userId, 5);
      ...
      if (memories.length > 0) {
        ...
        for (let i = 0; i < memories.length; i++) {
          ...
          yield chalk.gray(`  ${i + 1}. ${memory.content.substring(0, 100)}...\n`);
        }
      }
  ...
  async *execAsyncStream(shared) {
    ...
    if (this.framework.llm) {
      ...
      try {
        ...
        yield chalk.red(`❌ LLM generation error: ${error.message}\n`);
        shared.generatedResponse = `抱歉，我遇到了技术问题：${error.message}`;
      }
    } else {
      yield chalk.red('❌ No LLM available for response generation\n');
      shared.generatedResponse = '抱歉，LLM 系统不可用。';
    }
  }
}

/**
 * Framework-based Intelligent Agent Class
 */
export class FrameworkIntelligentAgent {
  constructor() {
    this.framework = null;
  ...
  async initialize() {
    ...
    console.log(chalk.blue('🚀 Initializing Framework-based Agent System...\n'));
    ...
    try {
      ...
      console.log(chalk.green('🎉 Framework Agent System initialized successfully!\n'));
  ...
  async processQuery(query, userId = 'default') {
    ...
    try {
      console.log(chalk.yellow('🔄 Executing framework workflow...\n'));
  ...
  async getStats() {
    ...
    const frameworkStats = this.framework.getStats ? this.framework.getStats() : {};
    const orchestrationStats = this.orchestration.getStats ? this.orchestration.getStats() : {};
  ...
  async close() {
    if (this.framework && this.framework.close) {
      await this.framework.close();
    }
    this.initialized = false;
    console.log(chalk.blue('👋 Framework Agent System closed'));
  }
}

export default FrameworkIntelligentAgent;
